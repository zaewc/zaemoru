...
export const ZaemoruHtmxComponents = zaemoruComponentTags;

export function createZaemoruHtmxAttrs(
...
export function bindZaemoruHtmx(root?: ParentNode) {
  const target = root ?? (typeof document === "undefined" ? undefined : document);
  if (!target) return;

  target.querySelectorAll("[data-zm-trigger]").forEach((el) => {
...
export function triggerZaemoruHtmxChange(element: Element) {
