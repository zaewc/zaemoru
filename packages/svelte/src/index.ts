...
export const ZaemoruSvelteComponents = zaemoruComponentTags;

type SvelteActionReturn = {
  update?: (props: ZaemoruAdapterProps) => void;
  destroy?: () => void;
};

export function createZaemoruSvelteAction(name: ZaemoruComponentName) {
  return function zaemoruSvelteAction(
    node: HTMLElement,
    props: ZaemoruAdapterProps = {},
  ): SvelteActionReturn {
    let cleanup = applyZaemoruProps(node, name, props);
    return {
      update(nextProps: ZaemoruAdapterProps = {}) {
        cleanup();
        cleanup = applyZaemoruProps(node, name, nextProps);
      },
      destroy() {
        cleanup();
      },
...
export function resolveZaemoruSvelteComponent(name: ZaemoruComponentName) {
  return zaemoruComponentTags[name];
}

export const zaemoruSvelte = {
  name: "zaemoru-svelte",
  customElementPrefix: "zm-",
  components: ZaemoruSvelteComponents,
};
