...
export const ZaemoruAngularComponents = zaemoruComponentTags;
export const ZaemoruAngularElementTags = Object.values(zaemoruComponentTags);

export function provideZaemoruElements() {
  return {
    schemas: [ZAEMORU_CUSTOM_ELEMENTS_SCHEMA],
    components: ZaemoruAngularComponents,
  };
}

export function createZaemoruAngularBinding(
  name: ZaemoruComponentName,
  props: ZaemoruAdapterProps = {},
) {
  return {
    tag: zaemoruComponentTags[name],
    attrs: toZaemoruAttributes(name, props),
  };
}
