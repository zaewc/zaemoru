...
type AlpineLike = {
  ...
  directive?: (
    name: string,
    callback: (el: HTMLElement, binding: { expression: string }, utilities?: unknown) => void,
  ) => void;
};

export const ZaemoruAlpineComponents = zaemoruComponentTags;

export function zaemoruAlpine(alpine?: AlpineLike) {
  alpine?.magic?.("zaemoru", () => ({
    components: ZaemoruAlpineComponents,
    attrs: toZaemoruAttributes,
  }));
}

export function createZaemoruAlpineAttrs(
...
export function bindZaemoruAlpineProps(
  node: HTMLElement,
  name: ZaemoruComponentName,
  props: ZaemoruAdapterProps = {},
) {
  return applyZaemoruProps(node, name, props);
}
